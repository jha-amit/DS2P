...
def node_val(my_type[:, ::1] Hash, my_type[:, ::1] Cost_horizontal, my_type[:, ::1] Cost_vertical, my_type[:, ::1] Cost_diag, my_type[::1] N, my_type K):
        ...
        for v2 in range(int(l1),int(l2)):
            ...
            if (j < 1 or j > K-1 or\
                ...
                if (abs(Hash[v2,0]-Hash[v1,0])<=1) :
                    ...
                    if Hash[v1,1]<=Hash[v2,1]: 
                        Cijk = Cost_horizontal[int(ip),int(jp)]+Cijk
                    else:
                        Cijk = Cost_vertical[int(ip),int(jp)]+Cijk

                elif ((abs(Hash[v2,0]-Hash[v1,0])<=2) and Hash[v1,0]>0 and  Hash[v1,0]<K-1):
                    ...
                    ip = 0.5*(j-i2)
                    jp = 0.5*(j+i2)
                    Cijk = Cost_diag[int(ip),int(jp)]+Cijk
